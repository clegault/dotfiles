...
local function streamdeck_discovery(connected, deck)
    ...
    if connected then
        local numberOfDevices = hs.streamdeck.numDevices()
        dbg("numberOfDevices: ")
        dbg(numberOfDevices)

        if numberOfDevices == 2 then
            dbg("Found 2 devices...")
            -- deck1 = hs.streamdeck.getDevice(1)
            deck2 = hs.streamdeck.getDevice(2)
            deck2:buttonCallback(streamdeck_button)
            deck2:reset()
            -- deck:buttonCallback(function(object, buttonID, pressed)
            --     dbg("object: %s, buttonID: %s, pressed: %s", object, buttonID, pressed)
            -- end)
        else
            dbg("Found 1 device...")
            deck1 = hs.streamdeck.getDevice(1)
            deck1:buttonCallback(streamdeck_button)
            deck1:reset()
            currentDeck = deck1
        end
